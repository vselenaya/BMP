...
typedef struct tagSECRET {  // структура для хранения сообщения, которое мы храним в изображении
    ...
    uint8_t* bits;  // указатель на массив битов, которыми кодируется сообщение
                    // (точнее: bits[i] - это элемент типа uint8_t, то есть он занимает 8 бит, и вот первые STEGO_BITS_CODE из 8 битов -
                    // - это биты, которые отвечают за кодировку символа string[i] сообщения, остальные 8-STEGO_BITS_CODE битов - нули)
} SECRET;           
